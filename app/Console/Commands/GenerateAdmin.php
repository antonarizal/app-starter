...
class GenerateAdmin extends Command
{
     ...
     * @var string
     */
    protected $signature = 'generate:admin {name : The name of the view file} {--layout= : The layout to use for the view} {--force : Overwrite the view file if it exists} {--table= : Create an inline view} {--model= : Create Model an inline view} {--timestamp= : true to add created_at and updated_at}';
     ...
     * @var string
     */
    protected $description = 'Command generate page in admin | php artisan generate:admin users --table=users --model=User';
    ...
    public function handle()
    {
            ...
            if(!$this->option('force'))
              ...
              if($this->option('table')){
                ...
                $tableColumn[$i] = $col != 'created_at' && $col != 'updated_at' ?"<flux:ui.table.column label=\"".Str::headline($col)."\" field=\"$col\" :sortField=\"\$sortField\" :sortDirection=\"\$sortDirection\" sortable=true ".$width." />" : '';
                $tableRow[$i] = $col != 'created_at' && $col != 'updated_at' ?"<flux:ui.table.cell>{{ \$row->$col }}</flux:ui.table.cell>" : '';
                  $i++;
                }
                $inputs = implode("\n",$input);
                $inputModals = implode("\n",$inputModal);
